...
## But What About Devices That Don't Support 802.1X?

Not all devices can perform 802.1X authentication. Think about:

- **Printers** that only have basic network stacks
- **IP cameras** with embedded firmware
- **IoT devices** like smart thermostats or door controllers
- **Legacy equipment** that predates 802.1X

Does this mean they're locked out of your network forever?

**No.** This is where **MAC Authentication Bypass (MAB)** comes in.

### How MAB Works

MAB is a fallback authentication method that uses a device's MAC address as its identity:

1. Device connects to the switch port
2. Switch waits for 802.1X response (typically 30 seconds)
3. No 802.1X response? Switch automatically tries MAB
4. Switch sends the device's MAC address to ISE as a username/password
5. ISE checks if that MAC address is authorized
6. If approved, the port opens

```
Authentication Flow with MAB Fallback:

Port connects
    ↓
Try 802.1X first
    ↓
Timeout (no 802.1X capability)?
    ↓
Fall back to MAB
    ↓
Send MAC address to ISE
    ↓
MAC in approved list? → Authorize
MAC unknown? → Deny or Guest VLAN
```

### Security Considerations

⚠️ **MAB is less secure than 802.1X** because:

- MAC addresses can be spoofed easily
- No encryption during authentication
- No user identity, only device identity

**Best practices:**
- Use MAB only when 802.1X isn't possible
- Place MAB devices in isolated VLANs
- Apply restrictive ACLs (only allow necessary protocols)
- Enable ISE profiling to auto-detect device types
- Monitor MAB authentications closely in ISE logs
